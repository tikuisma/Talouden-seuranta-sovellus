...
class Log_in_GUI:
    """
    Käyttöliittymän sisäänkirjautumisnäkymä, jossa voi joko luoda käyttäjä tai sisäänkirjautua.
    """
    def __init__(self): 
        self.user = ""
        ...
        self.log_in_button = Button(self.log_in_screen, text="Log in", command=self.login)
        self.log_in_button.place(x=220, y=120)

        self.lbl2 = Label(self.log_in_screen, text="Creating new user?\n Username length must be min. 4 and max. 12 marks.\n Write your username above to the open field\nand press ""Create new user""-button.")
        self.lbl2.place(x = 67, y = 200)
        self.new_user_button = Button(self.log_in_screen, text="Create new user", command=self.new_user)
